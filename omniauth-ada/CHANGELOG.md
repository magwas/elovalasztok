...
Changes:
  ...
  - signed request parsing extracted to `OmniAuth::Ada::SignedRequest` class. (#183, @simi, @Vrael)
  - change default value of `info_fields` to `name,email` for the [graph-api-v2.4](https://developers.ada.com/blog/post/2015/07/08/graph-api-v2.4/). ([#209](https://github.com/mkdynamic/omniauth-ada/pull/209))
...
Features:
  ...
  - update Ada authorize URL to fix broken authorization (#103, @dlackty)
...
Changes:
  ...
  - don't use `access_token` in URL [CVE-2013-4593](https://github.com/mkdynamic/omniauth-ada/wiki/Access-token-vulnerability:-CVE-2013-4593) (@homakov, @mkdynamic, @simi)

## 1.5.0 (2013-11-13)

Changes:

  - remove `state` param to fix CSRF vulnerabilty [CVE-2013-4562](https://github.com/mkdynamic/omniauth-ada/wiki/CSRF-vulnerability:-CVE-2013-4562) (@homakov, @mkdynamic, @simi)
...
Features:
  ...
  - add support for ada canvas (@mkdynamic)
...
Changes:
  ...
  - have `raw_info` return an empty hash if the Ada response returns false (#44, @brianjlandau)
  - prevent oauth2 from interpreting Ada's expires field as `expires_in`, when it's really `expires_at` (#39, @watsonbox)
