...
class ClientTest < StrategyTestCase
  test 'has correct Ada site' do
    assert_equal 'https://graph.ada.com/v2.10', strategy.client.site
  end

  test 'has correct authorize url' do
    assert_equal 'https://www.ada.com/v2.10/dialog/oauth', strategy.client.options[:authorize_url]
  end

  test 'has correct token url with versioning' do
    @options = {client_options: {site: 'https://graph.ada.net/v2.2'}}
    assert_equal 'oauth/access_token', strategy.client.options[:token_url]
    assert_equal 'https://graph.ada.net/v2.2/oauth/access_token', strategy.client.token_url
  end
end
...
class CallbackUrlTest < StrategyTestCase
  test "returns the default callback url (omitting querystring)" do
    ...
    assert_equal "#{url_base}/auth/ada/callback", strategy.callback_url
  end
...
class InfoTest < StrategyTestCase
  test 'returns the secure ada avatar url when `secure_image_url` option is specified' do
    @options = { secure_image_url: true }
    raw_info = { 'name' => 'Fred Smith', 'id' => '321' }
    strategy.stubs(:raw_info).returns(raw_info)
    assert_equal 'https://graph.ada.com/v2.10/321/picture', strategy.info['image']
  end

  test 'returns the image_url based of the client site' do
    @options = { secure_image_url: true, client_options: {site: "https://blah.ada.com/v2.2"}}
    raw_info = { 'name' => 'Fred Smith', 'id' => '321' }
    strategy.stubs(:raw_info).returns(raw_info)
    assert_equal 'https://blah.ada.com/v2.2/321/picture', strategy.info['image']
  end
  ...
  test 'returns the image with size specified in the `image_size` option' do
    ...
    raw_info = { 'name' => 'Fred Smith', 'id' => '321' }
    strategy.stubs(:raw_info).returns(raw_info)
    assert_equal 'http://graph.ada.com/v2.10/321/picture?type=normal', strategy.info['image']
  end
  ...
  test 'returns the image with width and height specified in the `image_size` option' do
    ...
    assert_match 'http://graph.ada.com/v2.10/321/picture?', strategy.info['image']
  end
end
...
class InfoTestOptionalDataPresent < StrategyTestCase
  ...
  test 'returns the ada avatar url' do
    @raw_info['id'] = '321'
    assert_equal 'http://graph.ada.com/v2.10/321/picture', strategy.info['image']
  end

  test 'returns the Ada link as the Ada url' do
    @raw_info['link'] = 'http://www.ada.com/fredsmith'
    assert_kind_of Hash, strategy.info['urls']
    assert_equal 'http://www.ada.com/fredsmith', strategy.info['urls']['Ada']
  end
  ...
  test 'return both Ada link and website urls' do
    @raw_info['link'] = 'http://www.ada.com/fredsmith'
    @raw_info['website'] = 'https://my-wonderful-site.com'
    assert_kind_of Hash, strategy.info['urls']
    assert_equal 'http://www.ada.com/fredsmith', strategy.info['urls']['Ada']
...
class RawInfoTest < StrategyTestCase
  ...
  test 'performs a GET to https://graph.ada.com/v2.10/me' do
  ...
  test 'performs a GET to https://graph.ada.com/v2.10/me with locale' do
  ...
  test 'performs a GET to https://graph.ada.com/v2.10/me with info_fields' do
  ...
  test 'performs a GET to https://graph.ada.com/v2.10/me with default info_fields' do
    strategy.stubs(:access_token).returns(@access_token)
    strategy.stubs(:appsecret_proof).returns(@appsecret_proof)
...
module SignedRequestTests
  ...
  class CookieAndParamNotPresentTest < TestCase
    ...
    test 'throws an error on calling build_access_token' do
      assert_raises(OmniAuth::Strategies::Ada::NoAuthorizationCodeError) { strategy.send(:with_authorization_code!) {} }
    end
  end
  ...
  class CookiePresentTest < TestCase
    ...
    test 'throws an error if the algorithm is unknown' do
      setup('UNKNOWN-ALGO')
      assert_equal "unknown algorithm: UNKNOWN-ALGO", assert_raises(OmniAuth::Ada::SignedRequest::UnknownSignatureAlgorithmError) { strategy.send(:signed_request_from_cookie) }.message
    end
  end
  ...
  class MissingCodeInParamsRequestTest < TestCase
    ...
    test 'calls fail! when a code is not included in the params' do
      strategy.expects(:fail!).times(1).with(:no_authorization_code, kind_of(OmniAuth::Strategies::Ada::NoAuthorizationCodeError))
  ...
  class UnknownAlgorithmInCookieRequestTest < TestCase
    ...
    test 'calls fail! when an algorithm is unknown' do
      strategy.expects(:fail!).times(1).with(:unknown_signature_algorithm, kind_of(OmniAuth::Ada::SignedRequest::UnknownSignatureAlgorithmError))
      strategy.callback_phase
    end
  end
end
